...
import { GetTransportLogUseCase } from '@usecases/transport/GetTransportLogUseCase';

describe('GetTransportLogUseCase', () => {
  ...
  it('returns an empty array if there is no log stored', async () => {
    ...
    const result = await new GetTransportLogUseCase().execute();
